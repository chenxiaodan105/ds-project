...
The author try to use ensemble methods to alleviate the harm caused by imbalance data. 
...
`sklearn ` has its own built-in method to recalculate the weights of different classes in the form `{class_label: weight}`. 
 ...
 Unfortunately, for this specific project, this method gives very little improvement for both the majority class 'no stance' and the minority class 'have stance', so it's not recommended here.
